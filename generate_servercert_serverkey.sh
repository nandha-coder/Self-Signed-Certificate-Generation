...
openssl req -newkey rsa:2048 -days 3600 -sha256 -subj "/C=$country/ST=$state/L=$locality/O=$organization/OU=$organizationalunit/CN=$commonname/emailAddress=$email" -nodes -keyout ${SERVER_KEY} -out ${SERVER_REQ}
sleep 1
echo "Rekeying......"
openssl rsa -in ${SERVER_KEY} -out ${SERVER_KEY}
sleep 1
echo "Generating the Signed Server Key and Server Certificate for $commonname"
openssl x509 -req -in ${SERVER_REQ} -days 3600 -sha256 -CA ${CA_CERT} -CAkey ${CA_KEY} -set_serial 01 -out ${SERVER_CERT}
